...
Run for p ∈ {4, 8, 12}. Results stable across all three lag orders.
...
The peak shifts from h=17 to h=10 and shrinks 22–44%. The shape changes qualitatively:
significant at h=0–2, insignificant at h=3–8, then a second rise peaking at h=10. This
non-monotone shape (rise–zero–rise) is robust across all three lag orders but is not
generated by any standard theoretical model we are aware of. No existing DSGE, search,
or RBC model produces an impulse response that dips to zero at intermediate horizons
then revives. Presenting this shape would require substantial defense.
...
All specs show a robust early trough at h=4–6 (−0.03 unscaled, p<0.01). The late-horizon
(h=12+) vacancy effects are less robust — the peak location flips between h=5 and h=18
depending on lag order. The early trough at h=4–6 is the robust vacancy feature.

**Asymmetry.** The persistence correction is asymmetric: unemployment's peak horizon
collapses 7 quarters, but the vacancy trough's early component (h=4–6) is essentially
unchanged. This rules out a clean "all persistence artifact" interpretation.

**Assessment.** The baseline LP overstates the peak horizon by at least 7 quarters due to
instrument persistence. The corrected unemployment IRF has a theoretically puzzling
non-monotone shape that would be difficult to defend. The corrected vacancy trough at
h=4–6 is a more credible and potentially matchable target for the model. See
[D10](decisions.md) for implications.
