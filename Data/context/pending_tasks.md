...
| E8 | ✅ **Bartik instrument persistence test (D10).** Implemented in `part5_lp.py` section [11], branch `instruments_LP_coefficient`. Ran p ∈ {4, 8, 12}. **Result:** δ→u peak shifts h=17→10, magnitude drops 22–44%; non-monotone rise–zero–rise shape robust across lag orders. δ→v early trough h=4–6 robust. Baseline LP overstates peak horizon by ≥7 quarters. **New problem:** corrected δ→u shape has no theoretical precedent — see [D10](decisions.md). | ✅ **completed Sept 22, 2026** |
...
| M8 | 🟡 **Hump-shape gap — partially resolved by E8.** Model peaks at h=1–2; baseline LP peaks at h=17 but this is inflated by instrument persistence. Corrected LP peaks at h=10 with a non-monotone shape (rise h=0–2, zero h=3–8, rise h=10). The h=10 peak is still beyond the model's h=2, but the gap is much smaller. The non-monotone shape has no theoretical precedent and would be difficult to defend. **Recommendation (D10):** target scale and sign in Block B, not the peak horizon. No new propagation mechanism needed if we adopt option (c). | ⚠️ **E8 complete; interpretation open — see D10** |
