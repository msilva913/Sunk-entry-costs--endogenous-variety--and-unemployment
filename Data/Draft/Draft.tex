...
\graphicspath{ 
	...
	{"C:/Users/msilv/Documents/Github/Sunk_entry_costs_endogenous_variety_unemployment"}
...
\begin{document}
	...
	available for new product-line formation, closing the loop. We segment the product and
	labor markets so that producers can employ a mass of workers, and thereby do not mechanically link employment and product lines one-for-one as \citet{schaal_2019}. Instead, $u_t$, $v_{t-1}$, and $N_t$ are jointly determined
...
\ref{app:diagnostics}.
	...
	than a feature of the data.  Appendix~\ref{app:filter_robustness} documents
...
The second set consists of empirically identified impulse responses from the
	...
	directly.
	The destruction shock has a monthly half-life of roughly two months,
	so prolonged unemployment responses to $\delta$ shocks reflect
	propagation through the firm and vacancy stocks rather than the
	persistence of the driving process itself.
 ...
 high-$\delta$ counterfactual sets $\delta_e=\tau$, and thus $s=0$, which replicates the \citet{coles_2018} setup in which all separations are from destruction events.
	...
	The markup $\mu=\varepsilon/(\varepsilon-1)$ is preserved in both arms;
	...
	Appendix~\ref{app:loglin} derives the log-linearized
	...
	four-fold increase in $\delta_e$ from baseline to the
	high-$\delta$ variant translates a given entry response into a
	commensurately larger accumulation of product lines.
	A complementary channel operates through the vacancy
	value. Log-linearising~\eqref{eq:Kdef} and using $\mathbb{E}_t\hat{Q}_{t+1}=\rho\hat{Q}_t$,
	...
	in~\eqref{eq:N_lom_ll} therefore favour the high-$\delta$
	...
	The high-$\delta$ variant therefore suffers a deeper $N$ trough on
	impact --- the destruction term is larger --- but also recovers faster,
	...
	same across both shocks: a model calibrated to the empirical ratio
	$\delta_e/\tau\approx10\%$ necessarily operates with a low-churning
	...
	First, $N_t$ itself has an ambiguous effect on $\chi_t^c$: a fall in $N$ raises
	...
	dominates the continuation-value elasticity ($\approx0.30$), so falling $N$
	tends to \emph{raise} $\chi^c$ --- a stabilizing feedback that partly offsets
	the initial $z$-driven decline.
	Second, the endogenous model calibrates a lower sunk entry cost $f_e$:
	with endogenous exit, steady-state profits are lower (some continuation
	costs are paid), so free entry requires a smaller $f_e$, amplifying entry
	responses at any given $\nu_f$.
	The exit-flow panel shows that the two models' exit flows are similar on
	impact but diverge at the peak, reflecting the net of these competing forces
	as they cumulate over the transition path; the elasticity
	...
	the interior-wage panel shows a smaller response in the no-variety arm at all
	horizons, and the gap is the pure $N\to\rho$ contribution. Because
	$w_t^{int}$ pins the job-creation surplus~\eqref{eq:jcc_eq}, a smaller
	$w_t^{int}$ response implies a smaller $\theta_t$ response, which
	attenuates both the unemployment rise and the vacancy fall.
	The $w_t^{int}$ gap is visible from impact---$z$ directly enters $w_t^{int}$
	even before $N$ adjusts, so the two lines diverge as the $N$ transition
	cumulates over the first few months.
	...
	fall more in the baseline than in the no-variety arm because the same $N$
...
Appendix~\ref{app:additional_comparisons}.
...
Appendix~\ref{app:loglin}, equations~\eqref{eq:K_decomp}--\eqref{eq:e_ll_delta}.
...
($\xi_{\mathrm{inv}}=0.1$, $\xi=10$), the same $\hat{Q}$ movement
generates a ten-fold larger $\hat{e}$ response, so entry collapses more
sharply.  The vacancy pool contracts further, $\theta$ falls more, and
...
at $\xi_{\mathrm{inv}}=0.1$, vacancies decline after an initial one-period
positive bump, and
the baseline at $\xi_{\mathrm{inv}}=1.0$ shows a stronger decline still.
