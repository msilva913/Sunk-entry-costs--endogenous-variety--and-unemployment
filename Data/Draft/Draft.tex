...
\graphicspath{ 
	...
	{"C:/Users/msilva913/Documents/GitHub/Sunk_entry_costs_endogenous_variety_unemployment/Data/Programs baseline"}
	{"C:/Users/msilva913/Documents/GitHub/Sunk_entry_costs_endogenous_variety_unemployment/Data/Bartek analysis/data/results"}
	{"C:/Users/m.silva/Documents/Github/Sunk_entry_costs_endogenous_variety_unemployment"}
	...
	{"C:/Users/m.silva/Dropbox/Documents - Copy/Research/Sunk vacancy costs endogenous product variety labor market frictions/Draft/figs"}
	{"C:/Users/m.silva/Dropbox/Documents - Copy/Research/Sunk vacancy costs endogenous product variety labor market frictions/Draft/figs"}
...
\begin{document}
	...
	Beveridge curve. Provided the cost of reposting vacancies is low relative to initial creation, firms repost after separation, so the aggregate vacancy stock partially self-corrects even while unemployment remains elevated. This  produces a transitory displacement along the Beveridge curve rather than a persistent  outward shift.	
	...
	formation. The two-way feedback between employment and product variety
	constitutes an aggregate demand externality in the sense of
	\citet{schaal_2019}: each new product line raises the welfare-based
	relative price $\rho(N_t)$, which enters recruiter compensation
	$w^{int}_t = \rho(N_t)z_t/\mu$ directly and raises the return to
	vacancy creation, while greater employment expands the resources
	available for new product-line formation, closing the loop. 
	
	We segment the product and
	labor markets so that producers can employ a mass of workers, and thereby do not mechanically link employment and product lines one-for-one. Instead, $u_t$, $v_{t-1}$, and $N_t$ are jointly determined
	in equilibrium without an accounting
	restriction.\footnote{In \citet{schaal_2019}, each matched
		...
		Euler equation. This separation is what allows $u_t$,
		$v_{t-1}$, and $N_t$ to be independent state variables.} Second, we endogenize firm exit, so that the aggregate destruction rate $\delta_t^e$ 
	...
	distinct and quantitatively important margin. \textit{First, establishment entry and exit constitute a large and cyclically sensitive component of gross job flows}. Table~\ref{tab:jf_t2_ext_2019} extends Table 2 in \citet{jaimovich_2008} through 2019Q4 using BLS Business Employment Dynamics (BED) data.\footnote{This table uses BED \emph{closings}, for comparability with \citet{jaimovich_2008}, whose Table 2 reports a mean closing share of 21.17 percent of quarterly gross job losses.  Their sample is 1992Q3--2005Q2 in the working paper version (SIEPR Discussion Paper 07-13).  Closings are the appropriate measure for their question, which concerns the number of firms actively competing in a period, and an establishment that is temporarily idle is not competing.  BED deaths were not published until May 2009, after their paper appeared, so the distinction was not available to them.  Our destruction measure is different.  The model's $\delta$ shock is the permanent elimination of a product line, so the analysis throughout the paper uses BED \emph{deaths}, constructed and defined in Section~\ref{sec:empirical_instr}.} The first three columns report the average fraction of quarterly gross job gains (losses) accounted for by opening (closing, dying) establishments. The next three report the ratio of the cyclical standard deviation of each flow to that of total flows, using the Hamilton (2018) regression filter ($h=8$, $p=4$) applied to raw levels. The final three repeat the calculation with the HP filter ($\lambda=1600$), which closely reproduces \citet{jaimovich_2008}'s benchmark of approximately $0.33$ over their original sample period. We stop the sample in 2019Q4. Extending it through the end of the BED data leaves the mean shares almost unchanged but collapses every cyclicality ratio, because the 2020 collapse inflates total job losses without a corresponding rise in permanent exit.
	
	The mean-share columns show that closing establishments account for $19.5$
	percent of gross job losses on average across all private-sector supersectors,
	a share that is stable across industries ranging from construction ($20.6$
	percent) to financial activities ($24.7$ percent). Dying establishments, the
	permanent subset, account for $12.0$ percent. The difference between the two is
	the temporary-shutdown component that the $\delta$ instrument must remove, and
	it is close to two-fifths of the closings margin. Permanent exit remains large
	in its own right. About one job loss in eight occurs at an establishment that
	never reopens.

	The relative standard deviation columns show that both margins move with the
	cycle, and that the permanent margin is the less volatile of the two. For total
	private, the closing flow moves at $0.239$ times the cyclical variation of
	total job losses under the Hamilton filter, against $0.187$ for deaths. The HP
	filter gives $0.223$ and $0.180$. The ordering holds in every supersector. The
	gap is widest in natural resources and mining ($0.205$ against $0.104$), where
	project-based and seasonal establishments suspend operations without exiting
	permanently. Elsewhere the two are close, so the cyclical sensitivity that
	\citet{jaimovich_2008} document is not an artifact of temporary shutdowns.
	Permanent exit is itself cyclical, which is what the $\delta$ shock requires.

	At $12.0$ percent of gross job losses, permanent exit is a minority of job
	destruction but far from a rounding error. This is what makes the
	$\delta$-versus-$s$ decomposition empirically tractable. The two margins are
	separately measurable, and they are distinct enough that conflating them
...
\end{table}
	...
	(probability $F(\chi_t^c)$). Both events occur before production and 
	...
	$\delta_{e,t}=1-(1-\delta_{t-1})F(\chi_t^c)$ and aggregate 
...
\tikzset{>=latex}
	\begin{figure}[H]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tikzpicture}[
				...
				{Exit if $\chi>\chi_t^c$\\Entry: $e_t{=}G(Q_t)$\\$N_t^e$ forms};
	...
	Section~\ref{sec:environment}. We then show that equilibrium comprises three blocks: a 
	...
	\begin{align}\label{eq:jcc}
		\kappa+\frac{K_t}{q(\theta_t)} = 
		\mathbb{E}_t m_{t+1}(1-\delta_{t})F(\chi_{t+1}^c)
	...
	production, the firm draws a continuation cost $\chi\sim F$ 
	in units of the consumption basket, where $F$ is a mixture 
...
The equilibrium laws of motion imply qualitatively distinct labor-market dynamics under product destruction versus match separation shocks.
This section establishes the asymmetry analytically, motivating the paper's focus on $\delta$ shocks throughout the quantitative analysis.

\begin{proposition}[$\delta$--$s$ asymmetry in unemployment-vacancy comovement]
	\label{prop:ds_asymmetry}
	\leavevmode
	\begin{enumerate}
		\item \emph{($s$ shock, exogenous exit.)}
		Suppose $p_0=0$. Then for any $\rho_s\in[0,1)$, a positive
		$s_t$ shock raises unemployment and vacancies simultaneously at
		horizon $h=1$, generating positive $u$--$v$ comovement.
		\item \emph{($s$ shock, endogenous exit.)}
		Under DS-CES with
		\begin{equation}\label{eq:gN_cond}
			f_e\bar{N} < \frac{\bar{z}(1-\bar{u})(\varepsilon-2)}{\varepsilon-1},
		\end{equation}
		the preceding result extends to the full model ($p_0>0$) if the
		reposting inflow $(1-\delta_t)\Lambda_{t+1}(1-u_t)$ weakly
		dominates the endogenous exit drain
		$(1-\delta_t)F'(\chi^c_{t+1})|\partial\chi^c_{t+1}/\partial s_t|
		\cdot[(1-q(\theta_t))v_t+s_t(1-u_t)]$
		on the pre-committed vacancy stock.
		The drain is increasing in $\rho_s$, so the condition holds for
		all $\rho_s\in[0,\bar\rho_s)$ for some $\bar\rho_s>0$.
		\item \emph{($\delta$ shock.)} A positive $\delta_t$ shock raises
		unemployment and reduces total vacancies at horizon $h=1$,
		generating negative $u$--$v$ comovement, provided entry does not
		fully offset pre-committed vacancy destruction:
		$\partial e_{t+1}/\partial\delta_t
		< |\partial v_{pre,t+1}/\partial\delta_t|$.
		A sufficient condition is that $\bar\delta_e/\bar\tau$ is small:
		when only a fraction of total separations flows through the
		destruction channel, duration shortening generates a weak entry
		cushion and the direct vacancy destruction dominates.
	\end{enumerate}
\end{proposition}
\noindent See \ref{app:proof_ds} for the proof.

Define $g(N,u)\equiv d^f(N,u)+f_e\rho(N)/\mu$ as the continuation
profit threshold: a firm survives whenever its fixed-cost draw satisfies
$\chi^c\leq g$.
Under DS-CES, $g_N$ balances two opposing forces: flow profit
$d^f\propto N^{(2-\varepsilon)/(\varepsilon-1)}$ combines a
market-share dilution effect ($1/N$) and a love-of-variety demand
expansion ($\rho(N)\propto N^{1/(\varepsilon-1)}$), with net exponent
$(2-\varepsilon)/(\varepsilon-1)$ negative for $\varepsilon>2$, so
dilution dominates and $d^f$ falls in $N$; and an option value of
entry $f_e\rho(N)/\mu\propto N^{1/(\varepsilon-1)}$, which always
rises in $N$.
Condition~\eqref{eq:gN_cond} ensures the profit decline dominates
the option-value rise ($g_N<0$, $\det J>1$): $\varepsilon>2$ is
necessary (it makes the right-hand side positive), while the bound
on $f_e\bar{N}$ ensures the option-value channel does not overturn
it. Both hold at standard calibrations.

From the vacancy law of motion~\eqref{eq:v_lom}, a positive $\delta_t$
shock raises $\delta_{e,t+1}$ and thereby destroys a fraction
$\hat\delta_{e,t+1}$ of the pre-committed vacancy stock
$\bar{v}^{pre}=\bar{v}-\bar{e}$; see Lemma~\ref{lem:vpre} in \ref{app:proof_ds}.
Whether \emph{total} vacancies fall depends on the entry response.
Duration shortening raises $K$~\eqref{eq:K_decomp} and hence the
vacancy value $Q$~\eqref{eq:Q_ll_delta}, stimulating entry
$e_{t+1}=G(Q_{t+1})$ that partially backfills destroyed vacancies.
Under free entry ($\xi\to\infty$, $Q\approx x_m$ pinned), $K$ rises
with $\delta_e$, the job-creation condition~\eqref{eq:jcc_eq} implies
$\theta$ falls, and total vacancies $v=\theta u$ have an ambiguous sign:
positive $u$--$v$ comovement requires the $u$ rise to exceed the
$\theta$ fall.


A sufficient condition for the $\theta$ fall to dominate is that
$\bar\delta_e/\bar\tau$ is small: when only a fraction of total
separations flow through the destruction channel, the proportional
$u$ rise from a $\delta$ shock is small while the duration-shortening
effect on $K$ is also correspondingly weak, so the $\theta$ decline
dominates.
At the empirically calibrated $\bar\delta_e/\bar\tau\approx 0.21$,
negative $u$--$v$ comovement holds even under free entry, as confirmed
\ref{app:comparison_D}; convex entry costs govern the
\emph{degree} of cushioning, not the \emph{sign} of comovement.
...
\begin{proposition}[Steady-state equilibria under DS-CES]
	...
	that $\delta_e$ is a parameter.  If $b>0$ or $\kappa>0$, the job
	creation curve \eqref{eq:curve_jcc} satisfies
	...
	\end{align}
	while the resource constraint curve \eqref{eq:curve_res_cons}
	satisfies $N_{RC}(0)=0$ and $N_{RC}(\theta)\to\bar{N}<\infty$.
	As $\theta\to\infty$, $N_{JC}(\theta)\to\infty$.  The two curves
	therefore cross an even number of times; generically exactly two
	equilibria exist in $(\theta,N)$ space, one with strictly higher
	tightness and product variety than the other.  
\end{proposition}
...
\ref{app:diagnostics}.
	...
	Proposition~\ref{prop:bgm_nest}. Relative to their calibration, we make two
...
The second set consists of empirically identified impulse responses from the
	...
	Proposition~\ref{prop:ds_asymmetry} turns on. The Bartik instrument is also
	...
	$\varepsilon^z_t = u^z_t/P_{11}$ and
	$\varepsilon^\delta_t = (u^\delta_t - P_{21}\varepsilon^z_t)/P_{22}$,
	so $\operatorname{Cov}(\varepsilon^z,\varepsilon^\delta)=0$ by
	construction.
	...
	Proposition~\ref{prop:ds_asymmetry} establishes that a positive
	$s$ shock generates weakly positive $u$--$v$ comovement ---
	the opposite of the Beveridge curve --- so the $s$ shock does not
	engage the vacancy-destruction and variety channels that are central
	to the model's amplification.
	The $s$ shock therefore provides a useful theoretical benchmark
 ...
 high-$\delta$ counterfactual sets $\delta_e=\tau$, and thus $s=0$, which replicates the \citet{coles_2018} setup in which all separations are from destruction events.
	...
	$\rho_t\equiv1$ regardless of $N_t$, removing the
	$N_t\to\rho_t\to w_t^{int}\to\mathrm{JCC}$ channel and
	...
	only the love-of-variety externality is shut off.
...
\ref{app:loglin} derives the log-linearized
	...
	coefficients,\footnote{Guess $\hat{Q}_t=\phi_Q\hat{\delta}_t$; then
	$\mathbb{E}_t\hat{Q}_{t+1}=\phi_Q\rho_\delta\hat{\delta}_t=\rho_\delta\hat{Q}_t$.
	...
	$\rho_t\downarrow\to w_t^{int}\downarrow\to\theta_t\downarrow\to u_t\uparrow$
	...
	is long-lived: $\rho_t$ stays depressed, $w_t^{int}=\rho_t z_t/\mu$
...
%	the shape visible in the empirical IRF of Section~\ref{sec:empirical}.
	...
	\end{align}
	where $\nu_{f,t}=\rho_t f_e/\mu$ from free entry~\eqref{eq:free_entry}.
	A negative $z$ shock lowers $Y_t^c = \rho_t z_t\bar{z}L_{c,t}$ immediately,
	...
	through $\rho_t=N_t^{1/(\varepsilon-1)}$ (elasticity $1/(\varepsilon-1)$).
	...
	$N_t$ depressed for longer, sustaining the $\rho_t\downarrow
	...
	The third comparison sets $\zeta=0$ so that $\rho_t\equiv1$
	...
	\begin{figure}[H]
		...
		\emph{negative} technology shock. Solid black: baseline
		($\rho_t = N_t^{1/(\varepsilon-1)}$, full variety).
		Dotted orange: no-variety counterfactual
		($\rho_t\equiv1$, $\zeta=0$; markup $\mu$ unchanged).
		Row~1 in percentage-point level deviation from steady state;
		rows~2--4 in $100\times\log$ deviation.}
		\label{fig:mechC_z}
	\end{figure}
	...
	the two models share the same direct $z\to w_t^{int}=\rho_t z_t/\mu$
	channel, but diverge through the indirect variety channel. In the
	baseline, $z\downarrow$ reduces entry, $N_t$ falls, $\rho_t=N_t^{1/(\varepsilon-1)}$
	falls, and $w_t^{int}$ declines by more than the direct $z$ impulse alone.
	With $\rho_t\equiv1$, the second-round compression of $w_t^{int}$ is absent,
	...
	$\rho_t$ has not yet moved, and $w_t^{int}=\rho_t z_t/\mu$ registers only
	...
	The $N$ and $N_e$ panels show a further distinction. With $\rho_t\equiv1$,
	...
	impulse destroys firms, $N_t$ falls, $\rho_t$ falls, and $w_t^{int}$
	...
	condition $\nu_f=\rho_t f_e/\mu$. Unemployment rises more and vacancies
	...
	Here, we can establish the relationship between $\pi_s$ and $\psi_c=\psi/(1+\psi)$. and the probability of a positive fixed cost $p_0$. Recall that the cutoff satisfies $\chi^c=Y^c[(\mu-1)/(\mu N)]+\rho f_e/\mu$. So, aggregate fixed costs are $X^c=\psi_c p_0(Y^c(\mu-1)/\mu+N\rho f_e/\mu)$.
	...
	We can rewrite the cutoff $\chi^c$ in terms of both the profit share and $\psi_c p_0$:
	\begin{align}
...
finally the vacancy-value chain.
...
From $w_t^{int} = \rho_t z_t/\mu$~\eqref{eq:recruiter_compensation} with
$\rho_t = N_t^{1/(\varepsilon-1)}$:
...
From $\tau_t = \delta_{e,t}+s_t(1-\delta_{e,t})$~\eqref{eq:tau_lom},
log-linearizing:
\begin{align}\label{eq:tau_ll}
    \hat\tau_t
    = \frac{\bar\delta_e}{\bar\tau}\,\hat\delta_{e,t}
    + \frac{\bar{s}(1-\bar\delta_e)}{\bar\tau}\,\hat{s}_t,
\end{align}
where the coefficients sum to one (Euler's theorem) since
...
\begin{align}\label{eq:u_ll}
    \tilde{u}_{t}
    = (1-\bar{f}-\bar\tau)\,\tilde{u}_{t-1}
    + (1-\bar{u})\,\tilde\tau_t
    - \bar{u}(1-\bar\tau)\,\tilde{f}_{t-1},
\end{align}
where $\tilde{x}_t = x_t-\bar{x}$ denotes the level deviation and
$\tilde{f}_t = f(\theta_t)-\bar{f}$.  The autoregressive coefficient
$(1-\bar{f}-\bar\tau)$ governs unemployment persistence: with monthly
...
\begin{align}\label{eq:v_ll}
    ...
    - \bar{v}^{\mathrm{pre}}\,\hat\delta_{e,t}
    + \tilde{e}_t,
\end{align}
...
($\mathbb{E}_t\hat{Q}_{t+1}=\rho_\omega\hat{Q}_t$, exact by undetermined
coefficients for shock $\omega\in\{z,\delta\}$ with persistence $\rho_\omega$):
\begin{align}\label{eq:Q_ll2}
    \hat{Q}_t
    = \underbrace{\frac{r+\bar\delta_e}{1+r-\rho_\omega(1-\bar\delta_e)}}_{\displaystyle\mathcal{D}(\bar\delta_e,\rho_\omega)}\hat{K}_t,
\end{align}
where $r$ is the steady-state discount rate.  The duration multiplier
$\mathcal{D}$ is strictly increasing in $\bar\delta_e$ (since
$\partial\mathcal{D}/\partial\bar\delta_e = (1+r)(1-\rho_\omega)/\mathrm{den}^2>0$
for $\rho_\omega<1$) and decreasing in shock persistence $\rho_\omega$.
...
\begin{align}\label{eq:e_ll}
    \hat{e}_t = \xi\,\hat{Q}_t = \xi\,\mathcal{D}(\bar\delta_e,\rho_\omega)\,\hat{K}_t,
\end{align}
...
Equation~\eqref{eq:Q_ll2} is derived holding $\delta_{e,t}$ fixed and so
applies exactly to $z$~shocks where $\hat\delta_{e,t}=0$ on impact.
For $\delta$~shocks, log-linearizing the full $K$ definition~\eqref{eq:Kdef},
$K_t=Q_t-\beta(1-\delta_{e,t})Q_{t+1}$, with $\delta_{e,t}$ treated as a
dynamic variable gives
\begin{align}\label{eq:K_decomp}
    \hat{K}_t
    \;=\;
    \underbrace{\frac{\bar\delta_e}{r+\bar\delta_e}\,\hat\delta_{e,t}}_{\displaystyle(+)\;\text{duration shortening}}
    \;+\;
    \underbrace{\frac{1}{\mathcal{D}(\bar\delta_e,\rho_\omega)}\,\hat{Q}_t}_{\displaystyle\text{vacancy value}}.
\end{align}
The \emph{duration-shortening} term is positive: a higher $\delta_{e,t}$
shortens the expected life of a filled vacancy, concentrating the asset's
value in the current-period dividend and raising $K$ for given $Q$.
Solving~\eqref{eq:K_decomp} for $\hat{Q}_t$ yields the extended
vacancy-value equation
\begin{align}\label{eq:Q_ll_delta}
    \hat{Q}_t
    \;=\;
    \mathcal{D}(\bar\delta_e,\rho_\omega)\,\hat{K}_t
    \;-\;
    \underbrace{\frac{\bar\delta_e}{1+r-\rho_\delta(1-\bar\delta_e)}}_{\displaystyle\phi_D(\bar\delta_e,\rho_\delta)>0}
    \hat\delta_{e,t},
\end{align}
which reduces to~\eqref{eq:Q_ll2} when $\hat\delta_{e,t}=0$.
The two terms act in opposite directions on $Q$: duration shortening raises
$K$ and hence $Q$, while the direct discount effect lowers the present value
of future dividends.  At the baseline calibration the first channel
dominates, so $\hat{K}^\delta>0$ and $\hat{Q}^\delta>0$.
Combining~\eqref{eq:Q_ll_delta} with~\eqref{eq:e_ll}, the entry response
to a destruction shock is
\begin{align}\label{eq:e_ll_delta}
    \hat{e}_t^\delta
    \;=\;
    \xi\,\mathcal{D}\,\hat{K}_t^\delta
    \;-\;
    \xi\,\phi_D\,\hat\delta_{e,t},
\end{align}
where the first term dominates, giving $\hat{e}^\delta>0$: entry expands
under a positive destruction shock.  For a technology shock the second
term is absent; a negative shock gives $\hat{K}^z<0$ via the JCC surplus
channel, so $\hat{e}^z<0$: entry contracts.  The signs therefore differ
across shocks --- $\hat{e}^\delta>0$, $\hat{e}^z<0$ --- and the role this
plays in shaping amplification is discussed in
...
entry expansion driven by duration shortening:
\begin{equation*}
    \hat\delta\uparrow
    ...
    \;\xrightarrow{\eqref{eq:K_decomp}}\; \hat{K}\uparrow
...
responses under the baseline ($\xi_{\mathrm{inv}}=1.0$, solid black) and a
near-free-entry counterfactual ($\xi_{\mathrm{inv}}=0.1$, dashed blue).
Both calibrations match the same empirical targets; $\xi_{\mathrm{inv}}$
...
vacancy value $Q$.  For destruction shocks, duration shortening raises $K$
and $Q$, generating an entry surplus that partially cushions the direct
vacancy destruction.  The parameter $\xi$ controls the strength of this
cushion: higher $\xi$ (lower $\xi_{\mathrm{inv}}$) amplifies the entry
...
the entry cushion.  This places a lower bound on $\xi_{\mathrm{inv}}$:
too close to free entry, and the model generates the wrong sign of
...
\begin{figure}[H]
    ...
    Solid black: baseline ($\xi_{\mathrm{inv}}=1.0$).
    Dashed blue: near-free-entry ($\xi_{\mathrm{inv}}=0.1$, recalibrated).
    Row~1 in percentage-point level deviation from steady state;
    rows~2--4 in $100\times\log$ deviation.}
    \label{fig:mechD_z}
\end{figure}
...
lowering $\xi_{\mathrm{inv}}$ substantially amplifies the labor-market
...
($\xi_{\mathrm{inv}}=0.1$, $\xi=10$), any given $\hat{Q}$ movement
...
effect of $\xi$ is reversed.  A positive $\delta$ shock triggers duration
shortening: the vacancy asset's expected life falls, concentrating value
in the current-period dividend and raising $K$~\eqref{eq:K_decomp}.  Via
the extended vacancy-value equation~\eqref{eq:Q_ll_delta}, $Q$ rises and
entry \emph{expands}: $\hat{e}^\delta>0$.  This entry expansion --- the
...
the decline in $\theta$ and the rise in $u$.  A larger $\xi$ increases cushioning, so the near-free-entry calibration generates \emph{less}
...
\emph{amplifies}.  A destruction shock shortens the expected life of a
product line; a vacancy attached to it becomes a shorter-duration claim,
which front-loads its value into the current dividend and \emph{raises}
$Q$.  Entry then leans against the shock, and a high $\xi$ makes matters
better --- entry \emph{cushions}.  Firms respond to a wave of destruction
...
not of $\xi$ alone.  To see this, consider the free-entry limit
$\xi_{\mathrm{inv}}\to 0$.  There $Q\approx x_m$ is pinned and
$K=x_m(r+\delta_e)/(1+r)$ rises with $\delta_e$ (duration shortening);
...
separations are not of the $\delta$ type --- and the duration shortening
effect on $K$ is correspondingly weak.  The $\theta$ decline therefore
dominates, and vacancies fall following destruction shocks
\emph{even at free entry}.  Figure~\ref{fig:mechD_delta} confirms this:
at $\xi_{\mathrm{inv}}=0.1$, vacancies rise for two months, turn negative
in the third, and trough at $-0.012$~pp, while the baseline at
$\xi_{\mathrm{inv}}=1.0$ shows a stronger decline still ($-0.018$~pp).
...
entry.  Convex entry costs ($\xi_{\mathrm{inv}}>0$) are not needed to
prevent the CK Beveridge curve shift; the low empirical $\bar\delta_e$
already rules it out.  The role of $\xi$ is then purely quantitative ---
...
At $\sigma=0$ the stochastic discount factor is
$m_t=\beta$, the system reduces to a scalar
expectational equation for $Q_t$ with characteristic
root $\beta(1-\bar\delta_e)<1$, and the unique
bounded solution follows directly by ruling out
explosive paths.
For $\sigma>0$, local uniqueness is maintained as
a standard regularity condition and verified
numerically as part of the baseline model solution.

\subsection*{Proof of Proposition~\ref{prop:bgm_nest}}

Set $p_0=0$. Then $F(0)=0$, so no firm pays a
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Proof of Proposition~\ref{prop:ds_asymmetry}
	and Lemma~\ref{lem:vpre}}
\label{app:proof_ds}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We first establish the Lemma on pre-committed vacancy destruction,
then prove each part of the Proposition.

\begin{lemma}[Pre-committed vacancy destruction]
\label{lem:vpre}
A positive $\delta_t$ shock unconditionally reduces the
pre-committed vacancy stock at $t+1$: 
\[
...
\[
	v_{pre,t+1}
	\;\equiv\;
	(1-\delta_{e,t+1})\bigl[(1-q(\theta_t))v_t
	+s_t(1-u_t)\bigr],
\]
where $(1-\delta_{e,t+1})=(1-\delta_t)\Lambda_{t+1}$ from
\eqref{eq:delta_e_lom}.
The bracket $[(1-q(\theta_t))v_t + s_t(1-u_t)]>0$ collects
unfilled vacancies from $t$ plus costless reposts from match
separations at end of $t$; both are predetermined with respect
to $\delta_t$ (which realizes at end of $t$, after $v_t$,
$\theta_t$, $u_t$ are determined). Hence,
\[
...
\begin{proof}[Proof of Proposition~\ref{prop:ds_asymmetry}]
\emph{Part~1: exogenous exit ($p_0=0$).}
Under $p_0=0$: $\Lambda_t=1$ and $\delta_{e,t}=\delta_{t-1}$.

\medskip\noindent\emph{Unemployment.}
The shock $s_t$ realizes at the end of period $t$.  From
\eqref{eq:tau_lom},
\[
	\tau_{t+1} = \delta_{e,t+1} + s_t(1-\delta_{e,t+1}),
\]
so $\partial\tau_{t+1}/\partial s_t = 1-\delta_{e,t+1} > 0$.
From the unemployment law of motion~\eqref{eq:u_lom},
\begin{align}\label{eq:u_lom_proof}
	u_{t+1}
	= (1-f(\theta_t))u_t
	  + \tau_{t+1}\bigl[(1-u_t)+f(\theta_t)u_t\bigr],
\end{align}
so $\partial u_{t+1}/\partial s_t
= (1-\delta_{e,t+1})\bigl[(1-u_t)+f(\theta_t)u_t\bigr] > 0$,
holding $\theta_t$ and other period-$t$ variables fixed.
Unemployment strictly rises on impact.

\medskip\noindent\emph{Vacancies.}
From the vacancy law of motion~\eqref{eq:v_lom},
\begin{align}\label{eq:v_lom_proof}
	v_{t+1}
	= \underbrace{(1-\delta_{e,t+1})
		\bigl[(1-q(\theta_t))v_t
...
\emph{Channel 1 (reposting):}
$s_t$ appears directly in $v_{pre,t+1}$ via the costless-reposting
term $(1-\delta_{e,t+1})s_t(1-u_t)$.
Since $s_t$ does not destroy current vacancies $v_t$ (which were
posted before $s_t$ realized) and does not reduce $(1-\delta_{e,t+1})$
(which depends on $\delta_t$, not $s_t$),
\[
	\frac{\partial\,v_{pre,t+1}}{\partial s_t}
	= (1-\delta_{e,t+1})(1-u_t) > 0.
\]

\emph{Channel 2 (entry):}
Entry $e_{t+1}=(Q_{t+1}/x_m)^\xi$ is chosen at the start of $t+1$;
we show $\partial Q_{t+1}/\partial s_t\geq 0$.

\emph{Sub-step (a): pre-entry tightness falls.}
Comparing the two derivatives established above,
\[
	\frac{\partial u_{t+1}}{\partial s_t}
	-\frac{\partial v_{pre,t+1}}{\partial s_t}
	= (1-\delta_{e,t+1})f(\theta_t)u_t > 0,
\]
so unemployment rises strictly more than the pre-committed vacancy
stock. Hence pre-entry tightness $\theta^{pre}_{t+1}\equiv
v_{pre,t+1}/u_{t+1}$ falls below its steady-state value $\bar\theta$,
implying $q(\theta^{pre}_{t+1})>\bar{q}$.

\emph{Sub-step (b): $K_{t+1}$ rises.}
Under $p_0=0$, the product-line LOM gives $N_{t+1}=(1-\delta_t)N_t$
(since $B_t=N_t$ from the business-formation block), so $N_{t+1}$ is
predetermined with respect to $s_t$. Consequently
$w^{int}_{t+1}=\rho(N_{t+1})z_{t+1}/\mu$ is also predetermined.
At $\rho_s=0$, future shocks revert to their mean, so the
right-hand side of the JCC~\eqref{eq:jcc_wage} at $t+1$---which
involves expected future $w^{int}$ and $s$---is invariant to $s_t$
at leading order, pinning $C_{t+1}\equiv\kappa+K_{t+1}/q(\theta_{t+1})$
near its steady-state value $\bar{C}$.
Therefore
\[
	K_{t+1}
	= \bigl(C_{t+1}-\kappa\bigr)q(\theta_{t+1})
	\approx (\bar{C}-\kappa)\,q(\theta_{t+1})
	> (\bar{C}-\kappa)\,\bar{q}
	= \bar{K},
\]
where the strict inequality uses $q(\theta_{t+1})>\bar{q}$, which
holds because for $\xi<\infty$ entry does not fully restore tightness
to $\bar\theta$.%
\footnote{Under free entry ($\xi\to\infty$), $Q_{t+1}=x_m$ is pinned,
$K_{t+1}=\bar{K}$, and $\partial e_{t+1}/\partial s_t=0$:
Channel~2 vanishes but Channel~1 remains strictly positive.}
It follows that $Q_{t+1}=K_{t+1}+\beta(1-\delta_{t+2})\mathbb{E}_{t+1}[Q_{t+2}]
>\bar{Q}$, so $\partial e_{t+1}/\partial s_t>0$.

Combining both channels:
$\partial v_{t+1}/\partial s_t > 0$ and
$\partial u_{t+1}/\partial s_t > 0$,
so unemployment and vacancies comove positively at $h=1$.
The argument applies recursively: at every subsequent horizon
$h\geq 2$, the $s$-shock does not enter the vacancy LOM with a
negative sign, while it continues to raise $\tau$ and hence $u$.
Positive $u$--$v$ comovement therefore holds for all $h\geq 1$.

\medskip
\emph{Part~2: endogenous exit ($p_0>0$).}
With $p_0>0$, $(1-\delta_{e,t+1})=(1-\delta_t)\Lambda_{t+1}$
where $\Lambda_{t+1}=F(\chi^c_{t+1})$ is the endogenous survival
probability.
The reposting channel becomes
\begin{align}\label{eq:dv_ds_full}
	\frac{\partial\,v_{pre,t+1}}{\partial s_t}
	= (1-\delta_t)\Lambda_{t+1}(1-u_t)
	  + (1-\delta_t)\frac{\partial\Lambda_{t+1}}{\partial s_t}
	  \bigl[(1-q(\theta_t))v_t+s_t(1-u_t)\bigr].
\end{align}
The sign of the second term is $\partial\Lambda_{t+1}/\partial s_t
= F'(\chi^c_{t+1})\cdot\partial\chi^c_{t+1}/\partial s_t$ with
$F'>0$; signing $\partial\chi^c_{t+1}/\partial s_t$ requires
accounting for the joint determination of $(\chi^c_{t+1},N_{t+1})$.

\medskip\noindent\emph{Joint system.}
From~\eqref{eq:N_lom_eq}:
\[
	N_{t+1} = (1-\delta_t)\,F(\chi^c_{t+1})\,B_t,
\]
where $B_t \equiv N_t + z_t(1-u_t)/f_e - Y^c_t/(f_e\rho(N_t))$
is predetermined (all period-$t$ quantities; entry decisions $e_t$
committed before $s_t$ realized are included in $B_t$ and face the
same exit draw $F(\chi^c_{t+1})$ at start of $t+1$).
Together with~\eqref{eq:cutoff_eq} and
$Y^c_{t+1}=\rho(N_{t+1})z_{t+1}(1-u_{t+1})$, the pair
$(\chi^c_{t+1},N_{t+1})$ solves
\begin{align}
	\chi^c &= g(N,\,u_{t+1}), \label{eq:eq1_proof}\\
	N      &= (1-\delta_t)\,F(\chi^c)\,B_t,\label{eq:eq2_proof}
\end{align}
where $g(N,u)\equiv\rho(N)z(1-u)(\mu(N)-1)/(\mu(N)N)+f_e\rho(N)/\mu(N)$.

\medskip\noindent\emph{Role of the Jacobian.}
The Jacobian of~\eqref{eq:eq1_proof}--\eqref{eq:eq2_proof} is
\[
	J = \begin{pmatrix}1 & -g_N \\
	    -(1-\delta_t)F'(\chi^c)B_t & 1\end{pmatrix},
	\quad
	\det J = 1-(1-\delta_t)F'(\chi^c)B_t\,g_N.
\]
The only channel through which $s_t$ enters is $u_{t+1}$;
$g_u\equiv\partial g/\partial u=-\rho(N)z(\mu-1)/(\mu N)<0$,
so the exogenous shift to~\eqref{eq:eq1_proof} is
$-g_u\,\partial u_{t+1}/\partial s_t>0$.
By the implicit function theorem:
\[
	\frac{\partial\chi^c_{t+1}}{\partial s_t}
	= \frac{g_u}{\det J}\cdot\frac{\partial u_{t+1}}{\partial s_t}.
\]
Under DS-CES, $g_N\propto z(1-u)(2-\varepsilon)/(\varepsilon-1)+f_eN$,
so condition~\eqref{eq:gN_cond} is equivalent to $g_N<0$, giving
$\det J=1-(1-\delta_t)F'(\chi^c)B_t\,g_N>1$.
This signs $\partial\chi^c_{t+1}/\partial s_t$ negative: it rules out
the case in which feedback from lower $N$ to higher per-firm
profitability reverses the direct employment channel $g_u<0$.
With $\det J>0$:
\[
	\frac{\partial\chi^c_{t+1}}{\partial s_t}<0,
	\qquad
	\frac{\partial\Lambda_{t+1}}{\partial s_t}
	= F'(\chi^c_{t+1})\,\frac{\partial\chi^c_{t+1}}{\partial s_t}<0.
\]
A positive $s$ shock raises endogenous exit, so the second term
in~\eqref{eq:dv_ds_full} is negative: the endogenous exit channel
opposes the reposting effect.

\medskip\noindent\emph{Sufficient condition.}
The drain is bounded by
$F'(\chi^c_{t+1})\cdot|\partial\chi^c_{t+1}/\partial s_t|
\cdot[(1-q(\theta_t))v_t+s_t(1-u_t)]$, where
$|\partial\chi^c_{t+1}/\partial s_t|=|g_u/\det J|
\cdot\partial u_{t+1}/\partial s_t$.
This drain is increasing in $\rho_s$: the option value component
$f_e\rho(N_{t+1})/\mu(N_{t+1})$ in~\eqref{eq:cutoff_eq} is
determined by the business formation Euler~\eqref{eq:N_euler_eq},
which depends on expected future $Y^c$.
At $\rho_s=0$, future employment reverts to $\bar s$ immediately,
leaving the option value unaffected by $s_t$; only the current-period
profit term $Y^c_{t+1}(\mu-1)/(\mu N_{t+1})$ falls.
At $\rho_s>0$, expected future $Y^c$ also falls (persistent lower
employment), reducing the option value and amplifying
$|\partial\chi^c_{t+1}/\partial s_t|$.
The drain is therefore minimized at $\rho_s=0$, so if the reposting
channel dominates there, it dominates for all
$\rho_s\in[0,\bar\rho_s)$ by continuity.
The entry channel (Channel~2 from Part~1) is non-negative throughout.

\medskip
\emph{Part~3: $\delta$ shock.}

\emph{Unemployment.}
Consider a positive $\delta_t$ shock with $s_t$ fixed at its
steady-state value $\bar{s}$ (a pure destruction shock).
Then $\delta_{e,t+1}$ rises, so
$\partial\tau_{t+1}/\partial\delta_t
= \partial\delta_{e,t+1}/\partial\delta_t
  \cdot(1-\bar{s})>0$,
and the same unemployment argument as Part~1 yields
$\partial u_{t+1}/\partial\delta_t>0$.

\emph{Vacancies.}
By Lemma~\ref{lem:vpre},
\[
  \left|\frac{\partial v_{pre,t+1}}{\partial\delta_t}\right|
  = \Lambda_{t+1}\bigl[(1-q(\theta_t))v_t + \bar{s}(1-u_t)\bigr]
  \;\xrightarrow{\text{s.s.}}\;
  (1-\bar{q})\bar{v} + \bar{s}(1-\bar{u}) > 0,
\]
a quantity bounded away from zero for any $\bar\delta_e/\bar\tau\in(0,1)$.
The entry response partially cushions this: from the
$K$-decomposition~\eqref{eq:K_decomp}, the duration-shortening
contribution to $\hat{K}^\delta$ carries coefficient
$\bar\delta_e/(r+\bar\delta_e)$, so
\[
  \frac{\partial e_{t+1}}{\partial\delta_t}
  \;\propto\;
  \frac{\bar\delta_e}{r+\bar\delta_e}
  \;\to\; 0
  \quad\text{as}\quad
  \bar\delta_e/\bar\tau\to 0.
\]
For $\bar\delta_e/\bar\tau$ sufficiently small, the entry cushion
is therefore strictly less than the pre-committed vacancy destruction,
so $\partial v_{t+1}/\partial\delta_t<0$, establishing
negative $u$--$v$ comovement at $h=1$.
\end{proof}
...
Throughout, restrict to DS-CES preferences: $\rho(N)=N^{1/(\varepsilon-1)}$
and $\mu=\varepsilon/(\varepsilon-1)$ constant.  Set $p_0=0$ so that
$\delta_e\in(0,1)$ is a parameter.  Write $h(\theta)\equiv f(\theta)/q(\theta)=\theta$
for the tightness ratio, and recall $L(\theta)=(1-\delta_e)f(\theta)/[\tau+(1-\delta_e)f(\theta)]$,
which is strictly increasing, concave, and satisfies $L(0)=0$ and
$L(\infty)=1-u_{\min}<1$.

\medskip\noindent\textbf{Step 1: $N_{JC}(\theta)$ is well-defined and strictly increasing.}

Under DS-CES the JCC \eqref{eq:curve_jcc} can be written as
\begin{align}\label{eq:jcc_rearranged}
  \frac{z}{\mu}\,N^{1/(\varepsilon-1)}
  = K(\theta) + b
  + \frac{1}{(1-\delta_e)(1-\phi)}
    \left(\kappa+\frac{K(\theta)}{q(\theta)}\right)
    \bigl(r+\tau+(1-\delta_e)\phi\,\theta q(\theta)\bigr)
  \equiv \Phi(\theta).
\end{align}
The right-hand side $\Phi(\theta)$ depends only on $\theta$: $K(\theta)$,
$q(\theta)$, and $\theta q(\theta)$ are all functions of tightness alone.
Since $1/(\varepsilon-1)>0$, inverting gives a unique
\begin{align}
  N_{JC}(\theta)=\left(\frac{\mu\,\Phi(\theta)}{z}\right)^{\varepsilon-1}.
\end{align}
$\Phi(\theta)$ is strictly increasing in $\theta$ (each term is non-decreasing
and $K(\theta)$ is strictly increasing because $e(\theta)$ is), so
$N_{JC}(\theta)$ is strictly increasing.
...
\begin{align}
  N_{RC}(\theta)
  =\frac{(\mu-1)z(1-\delta_e)}{f_e(\delta_e\mu+r)}\,L(\theta)
  \equiv \bar{N}\,\frac{L(\theta)}{L(\infty)},
\end{align}
where $\bar{N}=(\mu-1)z(1-\delta_e)L(\infty)/[f_e(\delta_e\mu+r)]<\infty$.
Since $L(\theta)$ is strictly increasing, $N_{RC}(\theta)$ is strictly
increasing, concave, with $N_{RC}(0)=0$ and $N_{RC}(\theta)\uparrow\bar{N}$.

\medskip\noindent\textbf{Step 3: Boundary behavior.}

\emph{At $\theta=0$.}
As $\theta\to 0$: $e(\theta)\to 0$, $K(\theta)\to 0$, and
$q(\theta)\to q_0>0$ (the vacancy-filling rate at zero tightness).  Hence
\begin{align}
  \Phi(0) = b + \frac{\kappa(r+\tau)}{(1-\delta_e)(1-\phi)},
\end{align}
so that
\begin{align}
  N_{JC}^0\equiv N_{JC}(0)
  =\left(\frac{\mu}{z}
    \left(b+\frac{\kappa(r+\tau)}{(1-\delta_e)(1-\phi)}\right)
  \right)^{\varepsilon-1}.
\end{align}
If $b>0$ or $\kappa>0$ then $N_{JC}^0>0=N_{RC}(0)$, so the JCC lies
\emph{strictly above} the RC curve at the origin.

\emph{As $\theta\to\infty$.}
$\Phi(\theta)\to\infty$ (since $K(\theta)\to\infty$), so
$N_{JC}(\theta)\to\infty$, while $N_{RC}(\theta)\to\bar{N}<\infty$.
The JCC therefore lies \emph{strictly above} the RC curve for all
sufficiently large $\theta$.

\medskip\noindent\textbf{Step 4: Even number of crossings; generically exactly two.}

Define $\Delta(\theta)\equiv N_{JC}(\theta)-N_{RC}(\theta)$.  Steps 1--3
establish $\Delta(0)=N_{JC}^0>0$ and $\Delta(\theta)\to+\infty$.  If
$\Delta(\theta)>0$ for all $\theta$, the system has no equilibrium.  For
a non-degenerate parameterization in which at least one equilibrium exists,
there must be some $\theta^*>0$ with $\Delta(\theta^*)=0$.  Since
$\Delta(0)>0$ and $\Delta(\infty)>0$, the intermediate value theorem
requires $\Delta$ to cross zero an \emph{even} number of times, yielding
an even number of equilibria.

A crossing is non-degenerate (transversal) when $\Delta'(\theta^*)\neq 0$.
A tangency $\Delta(\theta^*)=\Delta'(\theta^*)=0$ is a codimension-two
condition in the parameter space (it simultaneously imposes two equations
on the model's parameters) and therefore fails for a generic open dense
set of parameterizations.  Excluding tangencies, the even-crossing
structure collapses to generically \emph{exactly two} equilibria, one
with lower $(\theta_L,N_L)$ and one with higher $(\theta_H,N_H)$.

\medskip\noindent\textbf{Step 5: Uniqueness as $\varepsilon\to\infty$.}

As $\varepsilon\to\infty$, $\mu\to 1$ and $\rho(N)=N^{1/(\varepsilon-1)}\to 1$
for any fixed $N$.  The JCC \eqref{eq:jcc_rearranged} becomes independent of $N$:
\begin{align}
  \frac{z}{\mu} = \Phi(\theta),
\end{align}
which determines a unique $\theta^*$ independently of $N$.  The JCC is then
a vertical line in $(\theta,N)$ space; it crosses the strictly increasing
$N_{RC}(\theta)$ exactly once.  Hence a unique equilibrium obtains.
