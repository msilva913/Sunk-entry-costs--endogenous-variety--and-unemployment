...
\end{proposition}
	...
	with 25th--75th industry percentile bands.  The Deaths/Closings ratio
	...
	\begin{figure}[h]
		...
		quarter, with 25th--75th industry percentile bands.  Grey shading
		denotes NBER recessions.  \emph{Bottom panel:} Deaths/Closings
		ratio; dashed line at 1.0 is the theoretical upper bound.}
		\label{fig:bed_deaths}
	\end{figure}

	The Deaths/Closings ratio exhibits substantial heterogeneity across
	sectors.  Construction has the lowest mean ratio (0.57), consistent
	with the prevalence of project-based establishments that close on
	contract completion and reconstitute under a new entity --- a genuinely
	temporary exit from BED's perspective.  Leisure and hospitality is
	similarly low (0.82), reflecting seasonal closures.  At the other end,
	manufacturing (1.00 at the cap), information (1.00), and
	finance and real estate (1.00) hit the upper bound, implying that
	essentially all recorded closings in those sectors constitute permanent
	exits.\footnote{The Deaths/Closings ratio exceeds unity for manufacturing
	and information in certain years when computed from BDS exit counts
	rather than BED Deaths directly.  The BDS-implied ratio can exceed one
	due to a timing mismatch: BDS records employment at the March reference
	date \emph{prior} to exit while BED records employment at the time of
	the closing event, and establishments that permanently exit typically
	downsize before formal closure.  A secondary source is industry
	reclassification: a manufacturer converting to distribution contributes
	a BDS death to its origin sector but may not appear as a BED closing.
	Neither issue arises with BED Deaths, which is constructed entirely
	within the BED survey.  As a back-of-envelope verification, the
	employment-weighted mean Deaths/Closings ratio from BED (0.90,
	2001--2019) closely matches the mean permanence ratio implied by
	cross-walking BDS annual exits against BED quarterly closings
	($\bar{\pi} = 0.91$ after capping at unity), confirming that the two
	series measure the same underlying concept.  Full industry-level
	BDS-implied permanence ratios $\pi_{j,y}$ are reported in
	Appendix~\ref{app:perm_table}.}
	Wholesale trade (0.97), retail (0.98), and professional and business
	services (0.97) are near the cap, reflecting structurally high
	permanence in those sectors.  The employment-weighted mean across all
	12 supersectors is 0.90 (2001--2019 sample).
	...
	approximately 0.54 on raw rates to 0.42.\footnote{As a robustness check we further augment equation~\eqref{eq:resid_v2}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.88\textwidth]{instrument_delta_distribution.png}
		\caption{Cross-state distribution of the residualized $\delta$ Bartik
			instrument $\tilde{B}^{\delta}_{s,t}$ across 50 states,
			2001Q1--2019Q4.  Dark shading: interquartile range (25th--75th
			percentile); light shading: 10th--90th percentile; solid line:
			cross-state mean.  The instrument is constructed from BED
			establishment deaths residualized on aggregate productivity growth
			and lagged industry value-added growth
			(equation~\eqref{eq:resid_v2}), aggregated via fixed 2006
			employment shares (equation~\eqref{eq:bartik_delta}).  Grey
	...
	deviation $\hat\sigma_{\tilde{B}^\delta} = 14.0$ pp; all reported
	...
	projections are reported in Appendix~\ref{app:diagnostics}.
	...
	\begin{figure}[H]
		...
		\caption{Impulse responses of the state unemployment rate (left)
			...
			value-added growth (equation~\eqref{eq:resid_v2}).  Outcome
			window capped at 2019Q4 to exclude COVID.}
...
\ref{app:diagnostics}.
	...
	$r(\tilde{B}^\delta, \tilde{B}^{\text{LD}}) = 0.423$, down from $0.54$
	...
	Figure~\ref{fig:ld_uv_irf} reports the impulse responses of the
	unemployment and vacancy rates to a one-standard-deviation increase in
	$\tilde{B}^{\text{LD}}$.  The unemployment response rises monotonically
	to a peak of $+1.68$ pp at $h=20$, significant throughout.  The
	vacancy response is persistently negative (trough $-0.45$ pp at
	$h=20$).  Both patterns survive the severity placebo.  However, the
	monotonically rising unemployment IRF is anomalous given the estimated
	LD persistence $\hat\rho_{\text{LD}} \approx 0.3$: a transitory shock
	should produce a hump-shaped response, not a path that continues to
	rise two decades out.  This suggests the LD instrument captures
	variation beyond the model's $s$ shock --- most plausibly the GFC
	episode, in which layoffs were severe and prolonged.  We regard the LD
	IRFs as indicative rather than structural.

	\subsection{Joint local projection}\label{app:joint_lp}

	To assess whether the $\delta$ results are contaminated by the shared
	variation with LD, we estimate a joint local projection including both
	instruments as simultaneous regressors at each horizon $h$.  Three
	findings emerge.  First, the $\delta$ vacancy IRF is stable: coefficient
	differences relative to the separate specification are below $0.13$ pp
	at all $h$, confirming that the vacancy decline is not an artifact of
	LD exposure.  Second, the $\delta$ unemployment IRF shrinks by roughly
	$0.5$--$1.3$ pp in the joint specification, reflecting absorption of
	the variation shared with LD; the peak response remains positive and
	significant.  Third, the LD vacancy coefficient collapses to zero and
	is insignificant at every horizon once $\delta$ is controlled for.
	This zero reflects identification limits from the $r = 0.423$
	collinearity rather than structural evidence that separation shocks
	leave vacancies intact.  We treat the separate $\delta$ LP as the
	primary specification; the joint LP confirms robustness of the headline
	vacancy result.
