...
\ref{app:diagnostics}.
	...
	is stable across specifications: the joint and separate coefficients
	track within $0.13$ pp at every horizon $h$, and the confidence bands
	overlap throughout.  The vacancy decline following product-line
	destruction is not an artifact of the shared variation with LD.

	Second, the $\delta$ unemployment IRF shrinks by roughly $0.5$--$1.3$
	pp in the joint specification relative to the separate LP, reflecting
	absorption of the variation that $\tilde{B}^\delta$ and
	$\tilde{B}^{\text{LD}}$ share.  The peak response remains positive and
	significant, so the unemployment result is robust, if attenuated.
	...
	$r(\tilde{B}^\delta,\tilde{B}^{\text{LD}})=0.423$ is large enough that
