...
\begin{document}
	...
	Third, states with structurally higher establishment exit rates 
	...
	permanence-adjusted BED establishment closing rates --- scaling 
	quarterly closings by the fraction confirmed as permanent in the annual Business Dynamics Statistics to align the instrument with the model's concept of irreversible product-line destruction. The instrument weights 
	leave-one-out national industry-level closing rates by each state's 
...
are in Appendix~\ref{app:limiting_cases}.
...
\end{proposition}
	...
	\paragraph{Permanence calibration.}
	We address this by calibrating, for each supersector $j$ and BDS
	year $y$, the fraction of BED closings that proved permanent:
	\begin{equation}\label{eq:perm_ratio}
		\pi_{j,y} \;=\;
		\frac{\text{BDS exits}_{j,y}}
		     {\displaystyle\sum_{q:\,y(q)=y}\text{BED closings}_{j,q}}.
	\end{equation}
	The numerator is the Census Bureau Business Dynamics Statistics (BDS)
	count of establishments absent from March-$y$ payrolls that were
	present in March-$(y{-}1)$ --- the Census operational definition of
	permanent exit.  The denominator sums the four BED quarters mapping to
	BDS year $y$, which runs from Q2$(y{-}1)$ through Q1$(y)$.  The ratio
	$\pi_{j,y}$ is therefore the permanence rate: the share of BED closings
	in supersector $j$ during year $y$ that did not reopen.  The
	permanence-adjusted closing count entering the Bartik numerator is then
	$\text{closings}^{\text{perm}}_{j,t}=\pi_{j,y(t)}\times\text{BED
	closings}_{j,t}$.

	The calibration reveals substantial heterogeneity across sectors and
	time.\footnote{Appendix~\ref{app:perm_table} reports $\pi_{j,y}$ by
	supersector for the full sample.}  Construction has the lowest mean
	permanence rate ($\bar{\pi} = 0.56$), reflecting the high share of
	project-based establishments that dissolve at contract completion and
	reconstitute under a new entity --- BED records the dissolution but BDS
	records the continuation.  Leisure and hospitality is similarly low
	($\bar{\pi} = 0.79$), driven by seasonal closures.  Mining is a
	special case: the BLS Mining supersector combines NAICS 21 (mining and
	extraction) with a logging component, but BDS covers only NAICS 21; the
	resulting $\bar{\pi} \approx 0.18$ reflects the denominator's broader
	coverage rather than a genuine low permanence rate.

	Since permanence is a binary attribute of each closing, $\pi_{j,y}$
	is bounded above by unity by construction.  Nevertheless, the raw
	BDS/BED employment ratio mechanically exceeds unity in 145 of 360
	industry$\times$year cells (40 percent by employment weight), affecting
	ten of twelve supersectors.\footnote{The excess arises from a
	measurement timing difference between the two surveys: BDS records
	employment at an exiting establishment in the March reference period
	\emph{prior} to exit (year $y-1$), while BED records employment at the
	time of the quarterly closing event.  Establishments that permanently
	exit typically downsize before formal closure, so BDS employment
	(measured one year earlier) exceeds BED employment at closure; this is
	most pronounced in sectors with prolonged pre-exit decline such as
	Manufacturing and Information.  A secondary source is industry
	reclassification: an establishment that exits one NAICS supersector by
	converting to another activity (e.g.\ a manufacturer converting to
	distribution) contributes a BDS death to its origin sector but may not
	appear as a BED closing at all.  Both mechanisms produce a raw ratio
	that can exceed unity but carry no implications for instrument validity:
	they reflect measurement conventions, not a logical contradiction in the
	permanence concept.}  We cap $\pi_{j,y}$ at unity before applying the
	permanence adjustment.  For capped cells this is equivalent to treating
	all BED closings in that supersector-year as permanent --- a conservative
	assumption that, if anything, attenuates the permanence correction
	relative to the true structural rate.  The employment-weighted mean
	across industries and years after capping is $\bar{\pi} = 0.91$
	(2001--2019 sample), so the calibration matters primarily for sectors
	with systematic seasonal or project-cycle activity, and for years such
	as 2020 where temporary shutdowns were unusually prevalent.

	Most papers using BED data for shock identification
	\citep[e.g.,][]{moscarini_2012, elsby_2013} implicitly set
	$\pi_{j,y}=1$, treating all closings as permanent.  We are not aware
	of a prior paper applying a BDS-based permanence correction to align
	BED closings with the structural concept of irreversible product-line
	destruction.
	...
	\begin{equation}\label{eq:loo_rate}
		g^{\delta}_{-s,j,t}
		\;=\;
		\frac{\text{closings}^{\text{perm}}_{j,t}}
		     {E^{\text{nat}}_{-s,j,t_0}},
	\end{equation}
	...
	closings from the national count --- is not implemented because
	BED state-level closing counts by supersector are not publicly
	available.  Employment shares $\omega_{s,j}$ are drawn from the 2006
	...
	As a robustness check we further augment equation~\eqref{eq:resid_v2}
	...
	in Appendix~\ref{app:robustness}; v2 is the baseline throughout.
	...
	approximately 0.54 on raw rates to 0.42 --- and does not fall further
	under v3.  The persistence of this residual correlation after
	...
	\begin{figure}[H]
		...
		\caption{Impulse responses of the state unemployment rate (left)
			and log vacancy rate (right) to a one-standard-deviation increase
			in the residualised $\delta$ Bartik instrument
			($\hat{\sigma}_{\tilde{B}^\delta}=14.0$ pp, 2001Q1--2019Q4).
			...
			permanence-adjusted BED establishment closing rates
			(equation~\eqref{eq:bartik_delta}), residualised on aggregate
			productivity growth and lagged industry value-added growth
			(equation~\eqref{eq:resid_v2}).  Outcome window capped at 2019Q4
			to exclude COVID.}
		\label{fig:delta_uv_irf}
	\end{figure}

	% --- PLACEHOLDER: 4.3 Local projection specification to be drafted ---
	...
	state unemployment rate and log vacancy rate to a one-standard-deviation
	increase in the residualised $\delta$ Bartik instrument
	($\hat{\sigma}_{\tilde{B}^\delta} = 14.0$ pp, 2001Q1--2019Q4 sample).
	The unemployment response rises steadily from $+0.18$ pp at $h=0$ to
	a plateau of $+1.50$ pp by $h=17$--$20$, remaining highly significant
	throughout ($p < 0.001$ from $h=4$ onward).  The vacancy response falls
	persistently from $-0.06$ pp at $h=0$ to a trough of $-0.44$ pp at
	$h=14$, significant at $p < 0.01$ from $h=3$ onward and remaining so
	...
	medium horizons: the vacancy response stabilises around $h = 12$--$14$
	...
	creation resumes before the unemployment pool fully clears, and is
	further reinforced by the endogenous exit margin in the model.  As the
	impulse to $\delta_t$ decays --- with persistence $\rho_\delta \approx
	0.65$, the shock is largely dissipated by $h = 6$--$8$ --- entry of new
	product lines begins to restore vacancy creation, so the vacancy rate
	stabilises.  However, firms near the endogenous continuation threshold
	$\chi_t^c$ remain vulnerable to exit even after the impulse has passed,
	because their continuation values are still depressed by the lower product
	variety $N_t$ and the compressed markup.  These secondary exits extend
	the effective duration of the destruction episode, generating additional
	worker inflows into unemployment beyond what the exogenous $\delta$
	persistence alone would predict.  The unemployment stock therefore
	continues to accumulate --- each secondary exit inflows more workers ---
	while vacancies are simultaneously being replenished by new entrants.
	Formally, this is a stock-flow asymmetry: vacancies respond to
	\emph{incentives} (which recover roughly in line with the shock), whereas
	the unemployment stock drains only through \emph{matching flows}, which
	depend on tightness $\theta = v/u$.  Even as $v$ recovers toward its
	steady-state value, tightness remains depressed as long as the
	unemployment stock is large, slowing the drain.  The combination of
	endogenous secondary exit and the stock-flow asymmetry accounts for
	the divergence of the two IRFs at medium horizons.  Whether the
	...
	We estimate the model using a Bayesian simulated method of moments, building on
	\citet{coles_2018}, \citet{christiano_2016}, and \citet{colciago_2021}.
	The moment set extends the \citet{shimer_2005} labor-market suite to capture
	volatility, co-movement, and persistence in key variables, with special
	attention to separations and productivity: $m(Y)=[m_1'\; m_2'\; m_3']'$, where
	\begin{align}
		m_1 &= \begin{pmatrix}
			sd(u) & sd(v) & sd(\theta) & sd(x) & sd(\tau)
		\end{pmatrix}, \\
		m_2 &= \begin{pmatrix}
			\mathrm{Corr}(u,v) & \mathrm{corr}(u,\theta) & \mathrm{corr}(u,x) &
			\mathrm{corr}(u,\tau) & \mathrm{corr}(v,\theta) & \mathrm{corr}(v,x) &
			\mathrm{corr}(v,\tau)
		\end{pmatrix}, \\
		m_3 &= \begin{pmatrix}
			\mathrm{corr}(u,u_{-1}) & \mathrm{corr}(v,v_{-1}) & \mathrm{corr}(\theta,\theta_{-1})
			& \mathrm{corr}(x,x_{-1}) & \mathrm{corr}(s,s_{-1})
		\end{pmatrix}.
	\end{align}
	The Bayesian framework allows us to combine microeconomic priors with
	macroeconomic moments, quantify parameter uncertainty, and evaluate
	identification by comparing the posterior to the prior. For a given draw of
	estimated parameters, we simulate the model, compute model-implied moments,
	form the distance to the data, and update the posterior via MCMC. Marginal
	likelihoods facilitate model comparison for a fixed set of moments.
...
in the baseline analysis. \hfill$\square$
	...
	%	\hline
	%	Destruction rate $\delta$ & $\downarrow$ & $\downarrow$ & leftward & downward & Larger destruction rate increases the flow cost of opening vacancies $K$. For any given level of $N$, recruiters are less willing to open vacancies. Two possible interpretations for the AC curve: (1) Higher destruction rate results in fewer steady-state firms $N$; (2) Higher destruction rate results in more firm entry $\uparrow N_e$, which requires more employment in the market clearing condition. \\
	%	\hline
	%	Mass of idle recruiters $F$ & $\uparrow$ & $\uparrow$ & rightward & no movement & Higher mass of idle recruiters reduces the flow entry cost $K$, which allows active recruiters to post more vacancies for any given $N$. \\
	%	\hline
	%	Rate of time preference $r$ & $\downarrow$ & $\downarrow$ & leftward &  downward & Higher rate of time preference raises the flow entry cost and reduces the present value of the expected surplus, causing recruiters to post fewer vacancies for any given $N$. \\
	%	\hline
	%	Hiring cost $\kappa$ & $\downarrow$ & $\downarrow$ & leftward &  no movement & Increased hiring costs raise the cost of vacancy creation resulting in lower market tightness and fewer business lines.  \\
	%	\hline
	%\end{longtable}
	%\fontsize{12pt}{12pt}\selectfont
	%\subsection{Transmission of technology shocks}
	%\textcolor{red}{Not updated}
	%We derive elasticities that help interpret the transmission of technology shocks in the steady state. Use \eqref{eq:curve_res_cons} to write
	%\begin{align}
	%	w^{int}=z^{\frac{\varepsilon}{\varepsilon-1}}L(\theta)^{1/(\varepsilon-1)}\zeta_1
	%\end{align}
	%where $\zeta_1$ is a constant with respect to $z$ and $\theta$. Taking logs and differentiating yields
	%\begin{align}\label{eq:elast_wR_z}
	%	\epsilon_{w^{int},z}=\frac{\varepsilon}{\varepsilon-1}+\frac{1}{\varepsilon-1}\epsilon_{L,z}
	%\end{align}
	%From \eqref{eq:elast_wR_z}, variety effects enhance the direct effect of technology and introduce an additional channel through expansion of product lines. Indeed, as $\varepsilon\to \infty$, $\epsilon_{w^{int},z}\to 1$.
	%
	%Next, we derive an expression for $\epsilon_{L,z}$. Write $L$ in logs as 
	%$
	%\log L = \zeta_2 + (1-\eta_L)\log \theta-\log(\tau+(1-\delta)A\theta^{1-\eta_L})$
	%so that
	%\begin{align}
	%	\epsilon_{L,z}=(1-\eta_L)(1-L(\theta))\epsilon_{\theta,z}
	%\end{align}
	%The elasticity depends on the elasticity of the job finding rate $1-\eta_L$ multiplied by the unemployment rate. Putting this together yields
	%\begin{align}
	%	\epsilon_{w^{int},z}=\frac{\varepsilon}{\varepsilon-1}+\frac{1}{\varepsilon-1}(1-\eta_L)(1-L)\epsilon_{\theta,z}
	%\end{align}
	%To derive the full expression, start with
	%\begin{align}
	%	\frac{K}{q(\theta)} = \frac{1-\delta_e}{r+\tau}(w^{int}-w-K)
	%\end{align}
	%The elasticity of the left-hand side with respect to $z$ is $(\epsilon_{K,\theta}+\eta_L)\epsilon_{\theta,z}$. For the right-hand side, first apply derivatives
	%\begin{align}
	%	\der{(w^{int}-w-K)}{z} =(1-\phi)(\der{w^{int}}{z}-\der{K}{z}) -\frac{\phi}{1-\delta}\left(\der{\theta}{z}K+\theta \der{K}{z}\right)
	%\end{align}
	%Accordingly, we can write the elasticity in the right-hand side as
	%\begin{align}
	%	\epsilon_{w^{int}-w-K,z} = \frac{1}{w^{int}-w-K}\left[(1-\phi)\left(w^{int}\epsilon_{w^{int},z}-K\epsilon_{K,\theta}\epsilon_{\theta,z}\right)-\frac{\phi\theta K}{1-\delta}\epsilon_{\theta,z}(1+\epsilon_{K,\theta})\right]
	%\end{align}
	%\newpage
	%\subsection{Details of comparative statics}
	%\textcolor{red}{Temporarily comment out this section to preserve space}
	%In this section, we qualitatively characterize how steady state labor market tightness and firm product lines responds to various shocks.  We analyze the equilibrium in $(\theta,N)$ given by the system \eqref{eq:curve_jcc} and \eqref{eq:curve_res_cons}
	%\begin{align*}
	%\left(\kappa+\frac{K(\theta)}{q(\theta)}\right)\left(\rho+\tau+\phi \theta q(\theta)\right)&=(1-\delta)(1-\phi)\left(N^{1/(\varepsilon-1)}z/\mu-K(\theta)-b\right) \\
	%N &= \frac{(\mu-1)(1-\delta)z L(\theta)}{f_{e}(\delta \mu +\rho)}
	%\end{align*}
	%which is graphically represented in \ref{fig:steady_state_curves}.
	%\begin{figure}[h!]\label{fig:steady_state_curves}
	%	\centering
	...
	%\end{figure}
	%
	%Distinct from a standard labor search model, the job creation curve includes the flow cost of entry $K(\theta)$ due to finitely elastic vacancy creation.  Futhermore, the number of product lines $N$ is relevant to optimal job creation in that it affects the wage recruiters receive $w^{int}=pz/\mu$ through variety effects $p=N^{1/(\epsilon-1)}$.  Note that in the limiting case $\epsilon \rightarrow \infty$, in which variety effects are absent, the job creation curve would independently  determine equilibrium labor market tightness.  The resource constraint curve is essentially a market clearing condition which shows the equilibrium level of employment ($L(\theta)$) that a given number of business lines ($N$) can support.  
	%
	%First, consider a positive productivity shock $z$.  Higher productivity increases the real wage recruiters receive $w^{int}/p=z/\mu$, so for a given level of employment $L$ higher aggregate income can support a larger number of businesses.  This is represented by the AC curve shifting upward.  Similarly, higher wages $w^{int}$ increases the surplus received by recruiters; so for a given level of business lines $N$, there will be more vacancies creating causing a higher level of labor market tightness $\theta$.  This is represented by a rightward shift of the JC curve.  The new equilibrium features both higher tightness and business lines.
	%\begin{figure}[h!]
	%	\centering
	%	\includegraphics[scale=.3]{steady_state_z.png}
	%	\caption{Productivity shock: $z$}
	...
	%Next, consider an increase the the elasticity of substitution $\varepsilon$.  The value of $\varepsilon$ affects the real wage of recruiters through two distinct channels: the price ($p=N^{\frac{1}{\varepsilon-1}}$) and the markup ($\mu=\frac{\varepsilon}{\varepsilon-1}$).  Clearly $\partial \mu/\partial \varepsilon <0$ so that higher values of $\varepsilon$ increase the real wage of recruiters causing them to open more vacancies.  Conversely, $\partial p/\partial \varepsilon<0$ so that higher values of $\varepsilon$ decreases the wage recruiters receive; reflected by fewer vacancies.
	
	%You can show that the net effect on the wage, is dictated by the following inequalities:
	
	%\begin{align*}
	%\frac{\partial \left[ N^{\frac{1}{\varepsilon-1}}z\frac{\varepsilon-1}{\varepsilon} \right]}{\partial \varepsilon} > 0 \quad &\text{if} \quad %\frac{\varepsilon-1}{\varepsilon} > \ln N \\
	%\frac{\partial \left[ N^{\frac{1}{\varepsilon-1}}z\frac{\varepsilon-1}{\varepsilon} \right]}{\partial \varepsilon} < 0 \quad &\text{if} \quad \frac{\varepsilon-1}{\varepsilon} < \ln N \\
	%\frac{\partial \left[ N^{\frac{1}{\varepsilon-1}}z\frac{\varepsilon-1}{\varepsilon} \right]}{\partial \varepsilon} = 0 \quad &\text{if} \quad \frac{\varepsilon-1}{\varepsilon} = \ln N
	%\end{align*}
	%So the JC curve will rotate counter clockwise about the point $N=e^{\frac{\epsilon-1}{\epsilon}}$.   At high values of $N$, an increase in $\varepsilon$ will cause recruiters to realize lower wages; so that the price effect dominates.  At lower values of $N$ an marginal increase in $\varepsilon$ causes recruiters to realize higher wages; so that the markup effect dominates the markup effect. 
	
	%Only markups matter for the AC curve; so we can immediately conclude that higher values of $\varepsilon$ shift the AC curve rightward.  Economically, lower markups allows a higher level of employment which, in order to clear the labor market, corresponds to higher levels of tightness.  The net result is a reduction in steady state business lines $N$, and an ambiguous effect on tightness $\theta$.
	%\begin{figure}[h!]
	%	\centering
	%	\includegraphics[scale=.3]{steady_state_e.png}
	%	\caption{Elasticity  shock: $\epsilon$}
	%\end{figure}
	%
	%Next, consider  an increase in the mass of potential entrants $F$.  Higher $F$ only affects the job creation curve through the flow vacancy cost $K$, and clearly $\partial K/\partial F<0$ so that the job creation curve shifts rightward resulting in tighter labor markets and fewer business lines.
	%
	...
	%\end{figure}
	%
	%Finally, consider a positive shock to the rate of time preference $\rho$.  Higher rate of time preference raises the flow entry cost $K$ and reduces the present value of the expected surplus resulting in  fewer vacancies.  For any given level of tightness, higher rates of time preference increase the value of current product lines allowing higher levels of employment for any given $N$.  This is why the resource constraint curve shifts rightward.  The net result is lower equilibrium tightness and business lines represented as in \eqref{steady_state_s}.
	%
	%Below is a summary of the comparative statics:
	%\fontsize{9pt}{9pt}\selectfont
	%\begin{longtable}{|l|c|c|c|c|p{6cm}|}
	...
	%	\endfirsthead
	%	\hline
	%	Productivity $z$ & $\uparrow$ & $\uparrow$ & rightward & upward & Increase in the wage recruiters receive $w^{int}/p=z/\mu$ causes them to open more vacancies for any given level of $N$. Similarly, higher wages result in lower employment through market clearing. \\
	%	\hline
	%	Bargaining power $\phi$ & $\downarrow$ & $\downarrow$ & leftward & no movement & More bargaining power to the worker reduces the surplus that recruiters receive in the wage bargain. For a given level of $N$, they will open fewer vacancies. \\
	%	\hline
	%	Unemployment benefit $b$ & $\downarrow$ & $\downarrow$ & leftward & no movement & Higher unemployment benefits improve the workers' threat point in the wage bargain, resulting in less surplus to the recruiter. Hence, they are less willing to open vacancies for a given $N$. \\
	...
	%	Destruction rate $\delta$ & $\downarrow$ & $\downarrow$ & leftward & downward & Larger destruction rate increases the flow cost of opening vacancies $K$. For any given level of $N$, recruiters are less willing to open vacancies. Two possible interpretations for the AC curve: (1) Higher destruction rate results in fewer steady-state firms $N$; (2) Higher destruction rate results in more firm entry $\uparrow N_e$, which requires more employment in the market clearing condition. \\
	%	\hline
	%	Elasticity of substitution $\epsilon$ & $\downarrow$ & $\downarrow$ & leftward & no movement & Higher elasticity of substitution reduces markups thereby increasing wages, but also lowers prices causing lower wages. The positive effect on wages dominants for lower levels of business lines $N$, whereas the negative effect dominates for higher values of $N$.  Simultaneously, higher $\varepsilon$ allows for more employment and a tighter labor market from the AC curve. \\
	%	\hline
	%	Mass of idle recruiters $F$ & $\uparrow$ & $\uparrow$ & rightward & no movement & Higher mass of idle recruiters reduces the flow entry cost $K$, which allows active recruiters to post more vacancies for any given $N$. \\
	%	\hline
	%	Rate of time preference $\rho$ & $\downarrow$ & $\downarrow$ & leftward &  downward & Higher rate of time preference raises the flow entry cost and reduces the present value of the expected surplus, causing recruiters to post fewer vacancies for any given $N$. \\
	%	\hline
	%\end{longtable}
	%\fontsize{12pt}{12pt}\selectfont
