...
The second set consists of empirically identified impulse responses from the
	...
	The separation shock provides a useful theoretical benchmark:
	it raises $\tau$ and unemployment but triggers immediate vacancy
	reposting and leaves $N_t$ unchanged, generating symmetric and
	transient dynamics that do not engage the model's novel mechanisms.
...
shortening simultaneously generates a large entry surplus; and vacancies
comove \emph{positively} with unemployment --- the Beveridge curve shift
that is CK's central contribution.  In that model, avoiding
counterfactual positive comovement requires $\xi$ to be sufficiently
bounded away from free entry.
