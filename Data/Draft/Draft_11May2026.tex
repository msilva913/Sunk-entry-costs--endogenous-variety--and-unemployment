...
The second set consists of empirically identified impulse responses from the
	...
	exit share $\omega_\delta \equiv (1-F(x^c))/\delta_e$; the mass weight $p_0$ on
	...
	endogenous exit share $\omega_\delta=0.5$, which routes half of
	steady-state destruction through the continuation-cost margin.
	The exit-elasticity target is $\psi\zeta/(1-\zeta)=5$---the
	...
	survival quantile $\zeta\approx0.993$ at the calibrated exit
	...
	disciplined by a bivariate VAR(1) estimated on
	HP($\lambda=1{,}600$)-filtered log aggregate labor productivity
	and log aggregate exit rate over 1992Q3--2019Q4.
	Identification follows a Cholesky decomposition with technology
	ordered first, consistent with the model's within-period timing
	in which aggregate productivity is realized before firm-level
	continuation decisions; the identified destruction innovation is
	therefore orthogonal to contemporaneous technology.
	Converting quarterly autoregressive coefficients to monthly gives
	$\rho_z=0.902$ and $\sigma_z=0.0092$ for technology, and
	$\rho_\delta=0.592$ and $\sigma_\delta=0.0669$ for destruction,
	entering equations~\eqref{eq:ar1_z} and~\eqref{eq:ar1_delta}
	directly. The destruction shock has a monthly half-life of roughly
	two months, so prolonged unemployment responses to $\delta$ shocks
	reflect propagation through the firm and vacancy stocks rather
	than the persistence of the driving process itself.
	...
	at steady state: the baseline ($\delta_e=\bar\delta\ll\tau$)
	is contrasted with a high-$\delta$ variant ($\delta_e=\tau$,
	$p_0=0$) that replicates the \citet{coles_2018}
	timing assumption while holding all other calibration targets
	fixed. This comparison extends \citet{gabrovski_2025},
	who document the LOM-multiplier mechanism for technology shocks,
	to the $\delta$-shock dimension.
	...
	\emph{Comparison~C} (forthcoming) will set $\zeta=0$ so that
	...
	impulse responses to one-standard-deviation $z$ and $\delta$
	shocks. Each panel traces the transmission chain from labor
	...
	\begin{figure}[H]
		...
		technology shock. Solid black: baseline
	...
	Under the \emph{technology shock} (Figure~\ref{fig:mechA_z}),
	the high-$\delta$ variant generates a substantially larger
	response of $N_t$ despite a nearly identical response of the
	per-period vacancy dividend $K_t$.
	The mechanism is encoded in the firm law of
	motion~\eqref{eq:N_lom_eq}, which log-linearizes to
	...
	value. Equations~\eqref{eq:Kdef} and~\eqref{eq:entry_eq}
	imply that $Q_t$ is the present value of future $K_t$ flows
	discounted at rate $\beta(1-\delta_e)$, so a lower $\delta_e$
	implies a longer-duration vacancy asset with a larger $Q/K$
	ratio at steady state. The same proportional increase in $K_t$
	therefore produces a larger proportional increase in $Q_t$ and,
	through $Q_t=x_m e_t^{\xi^{-1}}$ in~\eqref{eq:entry_eq},
	a larger response of new vacancy postings $e_t$.
	In the high-$\delta$ variant, the shorter asset duration reverses
	this ordering, producing the larger $Q_t$ and $e_t$ responses
	visible in row~4 of Figure~\ref{fig:mechA_z}.

	Under the \emph{destruction shock} (Figure~\ref{fig:mechA_delta}),
	the comparison reverses in persistence.
	The high-$\delta$ variant absorbs the initial destruction with a
	deeper $N$ trough but recovers markedly faster.
	From~\eqref{eq:N_lom_ll}, the steady-state replacement rate
	$N^e/N=\delta_e/(1-\delta_e)$ is proportional to $\delta_e$,
	so a high-$\delta$ economy repairs destroyed product lines at
	a faster absolute rate each period.
	In the baseline, the monthly replacement rate is only
	$0.66\%$ of the firm stock.
	After a destruction impulse the variety depression is therefore
	long-lived: $\rho_t=N_t^{1/(\varepsilon-1)}$ stays depressed,
	the interior wage
	$w_t^{int}=\rho_t z_t\bar{z}/\mu$
	(equation~\eqref{eq:recruiter_compensation}) remains low,
	and the job creation condition~\eqref{eq:jcc_eq} stays tight,
	suppressing vacancy creation and prolonging the recovery of
	$\theta_t$ and $u_t$.
	In the high-$\delta$ variant the same chain operates at roughly
	four times the speed, compressing unemployment persistence and
	yielding faster convergence to steady state.
	The central quantitative message of Comparison~A is therefore
	that a model calibrated to the empirical ratio
	$\delta/\tau\approx10\%$ necessarily operates with a
	low-churning firm stock, which amplifies the persistence of
	destruction shocks through the $N$-recovery channel
	independently of any endogenous exit or variety mechanism.
	...
	The identification design equates $\bar\delta$ across
	specifications and holds $\tau$ fixed by raising $s$ in
	the exogenous model; by equations~\eqref{eq:N_lom_eq},
	\eqref{eq:v_lom}, and~\eqref{eq:u_lom}, this keeps
	steady-state $u$, $v$, and $\theta$ approximately equal so
	...
	\begin{figure}[H]
		...
		technology shock. Solid black: full baseline (endogenous exit,
		$\omega_\delta=0.5$, $\psi\approx0.033$). Dashed crimson:
	...
	the endogenous-exit model generates a smaller impact response
	and, more strikingly, a \emph{faster} recovery of unemployment
	than the exogenous model. The constrained specification
	propagates the $\delta$ shock more persistently. The explanation
	lies in the design constraint itself.
	...
	From~\eqref{eq:N_lom_ll} this halves the firm-stock replacement
	rate, so after the destruction impulse---which is identical
	across models by construction---the exogenous model's variety
	stock recovers at half the speed.
	The sustained depression of $N_t$ keeps $\rho_t$ and $w_t^{int}$
	low through~\eqref{eq:recruiter_compensation}, holding the job
	creation condition tight and prolonging unemployment.
	Greater persistence under exogenous exit is therefore a
	consequence of the low-churning firm stock that the $\tau$-fixing
	condition imposes on the exogenous model, not evidence against
	the endogenous exit channel.

	The dynamic contribution of the $x^c$ margin itself---through
	which a $\delta$ shock amplifies exit by reducing profitability
	and lowering the continuation threshold---is present but
	quantitatively modest at the calibrated $\psi\approx0.033$.
	The exit-flow panel ($\delta_{e,t}\times N_t$) shows that the
	endogenous and exogenous responses are nearly indistinguishable
	at impact, confirming that the $x^c$ amplification is dormant
	at the baseline calibration and that the dominant effect of
	endogenous exit in Comparison~B is the re-routing of destruction
	exposure through $\omega_\delta$ rather than dynamic feedback.
	Activating the dynamic $x^c$ channel more strongly would require
	$\psi$ of order unity, as in \citet{broer_2025}, which would in
	turn require relaxing the fixed-cost share target.

	For the \emph{technology shock} (Figure~\ref{fig:mechB_z}),
	the endogenous-exit model generates larger responses of $N_t$
	and $N_t^e$.
	Two forces contribute. First, a positive $z$ shock raises
	firm profitability, pushing up the continuation threshold $x_t^c$
	and reducing endogenous exit, reinforcing the $N_t$ expansion
	beyond what entry alone generates.
	Second, the endogenous model calibrates a lower sunk entry
	cost $f_e$, because the higher steady-state profit share under
	exogenous exit implies a larger $f_e$ through the free-entry
	condition~\eqref{eq:free_entry}; for the same signal from the
	variety Euler~\eqref{eq:euler_bf}, the endogenous model
	therefore supports a larger entry flow.
	The latter is a calibration artefact rather than a structural
	mechanism, so the $z$-shock comparison in Comparison~B should
	be interpreted with care.

	\medskip\noindent\textbf{Comparison C: variety effects} (forthcoming).
	The third comparison will set $\zeta=0$ so that $\rho_t\equiv1$
	...
	Under the full model, a $\delta$ shock depresses $N_t$, which
	lowers $\rho_t$, which reduces $w_t^{int}$, which tightens the
	job creation condition~\eqref{eq:jcc_eq} and simultaneously
	suppresses vacancy creation while unemployment rises---the
	two-sided Beveridge curve response that the empirical evidence
	in Section~\ref{sec:empirics} identifies.
	Setting $\zeta=0$ will remove this second-round amplification
	and quantify directly how much of the model's propagation of
	destruction shocks rests on the product-variety channel that
	the augmented Gabrovski-Silva framework cannot generate.
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
