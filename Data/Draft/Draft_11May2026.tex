...
\begin{document}
	\begin{frontmatter}
		\begin{abstract}
			How do business formation and exit dynamics influence job creation and destruction? We develop a unified model of endogenous business formation and exit and labor market frictions and study its implications for unemployment and vacancy dynamics. Monopolistically competitive producers use labor and face exit risk from product obsolescence and stochastic fixed costs. Creating businesses and vacancies both require sunk costs, making them partially predetermined: if a product line survives, unfilled vacancies are reposted. We show analytically that the business formation block primarily impacts the labor market via taste for diversity and endogenous exit. We also show that, under mild conditions, destruction shocks generative negative comovement of unemployment and vacancies, whereas match separation shock generate (counterfactual) positive comovement. When a firm exits, it immediately eliminates jobs and posted vacancies and reduces product variety, so vacancy persistence closely tracks product-line longevity. By contrast, match separation shocks destroy only filled jobs, generating counterfactual positive comovement of unemployment and vacancies.
			Adverse technology shocks induce exit among high-cost firms, amplifying labor-market effects. We estimate the model by Bayesian means on conditional moments--an impulse response of labor market variables to a destruction shock obtained via a Bartik instrument--and unconditional labor market moments.
			Quantitatively, the model matches the volatility, autocorrelation, and cross-correlation of unemployment, vacancies, business formation, employment-to-unemployment transitions, and establishment exit, with technology and destruction shocks playing the predominant roles.
		\end{abstract}
		%\begin{keyword}
...
\begin{proposition}[$s$ shock and positive $u$--$v$ comovement]
	\label{prop:ds_asymmetry}
	\leavevmode
	\begin{enumerate}
		\item Suppose $p_0=0$ (exogenous exit). Then
		for any $\rho_s\in[0,1)$, a positive $s_t$ shock raises
		unemployment and vacancies simultaneously at horizon $h=1$,
		generating positive $u$--$v$ comovement.
		\item The preceding result extends to the full model ($p_0>0$) if the
		...
		The drain is increasing in $\rho_s$: at $\rho_s=0$, only
		the current-period profit component of $\chi^c_{t+1}$ responds
		to $s_t$; at $\rho_s>0$, the option value component
		of~\eqref{eq:cutoff_eq} also falls, since persistent lower
		employment reduces expected future $Y^c$ and hence the entry
		option value.
		The condition therefore holds for all
		$\rho_s\in[0,\bar\rho_s)$ for some $\bar\rho_s>0$.
	\end{enumerate}
\end{proposition}
\noindent See \ref{app:proof_ds} for the proof.
%Section~\ref{sec:quantitative} verifies that the calibrated
%$\rho_s\approx 0.3$ lies within $[0,\bar\rho_s)$, confirming positive
%$u$--$v$ comovement under endogenous exit.
...
Under $p_0=0$: $\Lambda_t=1$, $\delta_{e,t}=\delta_{t-1}$, and the
equilibrium reduces to the AGS system of Proposition~\ref{prop:ags}.
...
Entry $e_{t+1}=(Q_{t+1}/x_m)^\xi\geq 0$ is chosen at the start of
$t+1$ knowing $s_t$; any entry response is weakly positive since $Q$
and $K$ are non-decreasing in $s_t$.
Specifically, higher $s_t$ raises $u_{t+1}$, lowering $\theta_{t+1}$;
lower $\theta$ raises $q(\theta)$ (easier to fill vacancies) and, via
the Nash wage equation~\eqref{eq:wage_eq}, lowers the worker's outside
option $f(\theta_t)(\kappa+K_t/q(\theta_t))$, reducing the Nash wage
$w_t$ and raising the firm surplus $w_t^{int}-w_t$.
Both effects increase $K_{t+1}$~\eqref{eq:Kdef} and hence $Q_{t+1}$,
stimulating entry.
...
\]
The Jacobian condition $\det J>0$---maintained as the standard
local-uniqueness assumption\footnote{Automatic when $g_N\leq 0$.
When $g_N>0$, requires $(1-\delta_t)F'(\chi^c)B_t\,g_N<1$.}---is
precisely what signs this expression negative: it rules out the
case where the feedback from lower $N$ to higher profitability
reverses the direct employment channel $g_u<0$.
With $\det J>0$:
\[
