...
---------------------------------------------------------------
    ...
    ⚠ NOTE: BLS does not publish state JOLTS via the public API v1/v2.
...
def fetch_jolts_vacancies_state(
    ...
    ⚠ AGGREGATION NOTE — STOCK measure:
        ...
        for batch in batches:
            ...
            try:
                ...
                if call_num == 1:
                    ...
                    print("  ⚠ First API call failed — check series ID format and "
    ...
    if not all_obs:
        print(
            "\n  ⚠ WARNING: BLS returned 0 vacancy observations for all state series.\n"
            "  Possible causes:\n"
...
def _dist_series(df, val_col, wt_col="labor_force"):
    ...
    wmean = weighted_mean(df.dropna(subset=[val_col]), val_col, wt_col).sort_index()
...
# Compute vacancy RATE = vacancies / labor_force * 100  (analogous to
# unemployment rate) so the lower panel is directly comparable in units.
# vacancies is in thousands (JOLTS units); labor_force is in actual persons
# (LAUS API returns persons, not thousands) — multiply vacancies by 1000
# to convert to the same persons unit before forming the ratio.
# -----------------------------------------------------------------------
_vac_plot = (
    "vacancies" in quarterly.columns
    and quarterly["vacancies"].notna().sum() > 0
)

if _vac_plot:
    quarterly["vac_rate"] = (
        quarterly["vacancies"] * 1000 / quarterly["labor_force"] * 100
    )
    ...
    _vac_act = quarterly["vacancies"] * 1000               # persons
...
# Explicit ticks at round values so labels are always legible.
# -----------------------------------------------------------------------
if _vac_plot:
