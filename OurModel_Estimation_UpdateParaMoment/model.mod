...
// Group 8: observables
    ...
    tau_obs = log(s_agg+s_agg(-1)+s_agg(-2));

    [name='Business destruction observation'] //
    delta_obs = log(exp(log_delta)+exp(log_delta(-1))+exp(log_delta(-2)));
...
sigma_delta,inv_gamma_pdf,  0.02, 0.01;
end;
...
stoch_simul(order=1,nofunctions,nograph,noprint)
    u_obs, v_obs, theta_obs, lp_obs, tau_obs, delta_obs, bf_obs;
stack_model_moments;
save results;
delete *.eps;