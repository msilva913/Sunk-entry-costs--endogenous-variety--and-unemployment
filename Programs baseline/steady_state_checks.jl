...
targets = (labor_share=0.66, dest_ann=0.10, r_ann=0.04, f =0.41, η_L=0.6, q=0.8, sep=0.031, b_ratio=0.71,
 hiring_cost_wages=0.42, ξ_inv=1/0.265, ε=4.3, σ=1.0, N=1.0, w=1.0)
cal = calibrate_labor_share(targets)

steady = steady_state(cal)

@unpack θ, p, L_c, L_e, w, w_int, L, N, N_e, K, q, v, ν_f, d_f, C, Y, Y_c, X_v, X, labor_share, 
...
@assert abs(X/(q*v*w) -hiring_cost_wages) < 1e-6
X_v/X
