...
% A, F, and s are dependent parameters and are thus commented out
    ...
    phi $\phi$ (long_name = 'Worker bargaining power')      
...
#sr_1 = (rho+tau)/(1-delta)*(1/q);
#sr_2 = ((rho+tau)/q+phi*theta)/((1-delta)*(1-phi));
#sr = sr_1/sr_2;
#w_R_net_K = (w_ss-sr*b)/(1-sr); //w_R-K
#K_ss = (w_R_net_K - b)/sr_2;
#w_R_ss = w_R_net_K + K_ss;
#z = (mu/p_ss)*w_R_ss;
...
K/q = beta*lam(+1)/lam*(1-delta)*(w_R(+1)-w(+1)-K(+1)+(1-s)*K(+1)/q(+1));
...
K = Q-beta*lam(+1)/lam*(1-delta)*Q(+1);
...
[mame = 'Firm value relative to price']
nu_f = p*f_e/mu;
...
steady_state_model;
    ...
    sr_1s = (rho_ss+tau)/(1-delta)*(1/q);
    sr_2s = ((rho_ss+tau)/q+phi*theta)/((1-delta)*(1-phi));
    srs = sr_1s/sr_2s;
    w_R_net_K_ss = (w_ss-srs*b_ss)/(1-srs); //w_R-K
    K = (w_R_net_K_ss - b_ss)/sr_2s;
    w_R = w_R_net_K_ss + K;
...
stoch_simul (order=1, nofunctions, irf=1000, periods=0)
