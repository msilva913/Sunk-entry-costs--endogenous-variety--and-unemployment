...
var //x           ${x}$ (long_name='technology')
    ...
    theta_delta     ${\theta_z}$ (long_name='technology shock')
...
varexo e_z ${e_z}$ (long_name= 'technology shock')
       e_delta ${e_delta}$ (long_name='firm destruction shock')
    ;
...
#surplus_ratio = (rho+tau)/(1-delta)*(1/(q_ss*x_v));
#K_ss = (w_R_ss-w_ss)/(1+surplus_ratio);
#kappa = (1-x_v)/x_v*K_ss/q_ss;

// From wage equation find ϕ
#phi = (w_ss-b)/(w_R_ss-K_ss+theta_ss/(1-delta)*(K_ss+q_ss*kappa)-b);
...
(kappa+K/q) = beta*lam(+1)/lam*(1-delta*exp(theta_delta))*(w_R(+1)-w(+1)-K(+1)+(1-s)*(kappa+K(+1)/q(+1)));

[name ='Marginal revenue product']
w_R = p*z*exp(theta_z)/mu;

[name = 'Wage equation']
w = phi*(w_R - K + theta/(1-delta*exp(theta_delta))*(K+q*kappa)) + (1-phi)*b;

[name = 'Value of a vacancy']
Q = (e/F)^(xi_inv);

[name = 'Expected discounted difference in vacancy value']
K = (Q-beta*lam(+1)/lam*(1-delta*exp(theta_delta))*Q(+1));
...
[name='Firm value']
nu_f = beta*(1-delta*exp(theta_delta))*lam(+1)/lam*(nu_f(+1)+d_f(+1));
...
u = (1-(1-delta*exp(theta_delta))*f(-1))*u(-1) + s_agg*(1-u(-1)); % u is predetermined

[name = 'LOM of firms']
N = (1-delta*exp(theta_delta))*(N(-1) + N_e(-1)); % N is predetermined
...
steady_state_model;
    ...
    surplus_ratio_ss = (rho_ss+tau)/(1-delta)*(1/(q*x_v));
    K = (w_R-w)/(1+surplus_ratio_ss);
    kappa_ss = (1-x_v)/x_v*K/q;
