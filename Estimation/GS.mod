...
var //x           ${x}$ (long_name='technology')
    ...
    theta_delta ${\theta_{\delta}}$ (long_name='destruction rate shock')
...
varexo e_x ${e_x}$ (long_name= 'technology shock')
       ...
       e_delta  ${e_{\delta}}$   (long_name = 'product destruction rate shock')
    ;
...
K/q = beta*exp(theta_b)*(1-delta*exp(theta_delta))*(exp(theta_x(+1))-w(+1)-K(+1)+(1-s)*K(+1)/q(+1));

[name = 'Expected discounted difference in vacancy value']
K = (e^(1/xi) - beta*exp(theta_b)*(1-delta*exp(theta_delta))*e(+1)^(1/xi))/(F^(1/xi));

[name = 'Wage equation']
w = alpha_L*exp(theta_alphaL)*(exp(theta_x) - K + theta/(1-delta*exp(theta_delta))*K) + (1-alpha_L*exp(theta_alphaL))*b;

[name = 'LOM of vacancies']
v = (1-delta*exp(theta_delta(-1)))*((1-q(-1))*v(-1) + s*(1-u(-1))) + e;

[name = 'LOM of unemployment']
u = (1-(1-delta*exp(theta_delta(-1)))*f(-1))*u(-1) + s_agg(-1)*(1-u(-1));
...
[name ='Destruction rate shock']
theta_delta = rho_delta*theta_delta(-1) + e_delta;
...
rho_delta,  0.9, 0.01, 0.999999,    BETA_PDF, 0.6, 0.2;
...
stderr e_delta, 0.01, 0.00001, 0.2,   INV_GAMMA_PDF, 0.01, 0.1;
...
//moments_varendo,
prior_trunc=0)
u_obs, v_obs, x_obs, s_obs, theta_x, u_imp, v_imp, theta_imp, e_imp;
...
stoch_simul (order=1, nofunctions, irf=80, periods=0, pruning)
