...
def construct_data(init, final, freq):
    ...
    " BFS "
    sbf1 = fred.get_series('BFBF4QTOTALSAUS')
    sbf2 = fred.get_series('BFBF8QTOTALSAUS')
    bf1 = (sbf1.resample('QE').mean().dropna()) / pop
    bf2 = (sbf2.resample('QE').mean().dropna()) / pop

    " Note: these series imply labor productivity in each sector "
    " List of data series "
    var_load_list = [c, cons_share, u, v, theta, f, lp, ls, s, w, bf1, bf2] 
